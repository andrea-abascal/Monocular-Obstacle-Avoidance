...
from depth_anything_v2.dpt_tr import DepthAnythingV2
...
def main():
    ...
    depth_anything.load_state_dict(torch.load(f'/home/andrea/Documents/Projects/MDE/modules/depth_estimation/checkpoints/depth_anything_v2_{args.encoder}.pth', map_location='cpu'))
    device = torch.device('cpu')
    depth_anything = depth_anything.to(device).eval()

    # Define dummy input data
    dummy_input = torch.ones((3, args.input_size, args.input_size)).unsqueeze(0)
    #dummy_input = torch.ones((1, 3, args.input_size, args.input_size),  device=device)
